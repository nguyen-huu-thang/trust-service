...
public final class IdService {
    ...
    private static final int ID_LENGTH = 20;        // 160-bit
    private static final int BASE62_LENGTH = 27;    // fixed length
    ...
    public static String toBase62(Id id) {
        String encoded = encodeBase62(id.toBytes());
        return leftPad(encoded, BASE62_LENGTH, '0');
    }

    public static Id fromString(String value) {
        if (value == null || value.length() != BASE62_LENGTH) {
            throw new IllegalArgumentException("Invalid Base62 length");
        }

        byte[] decoded = decodeBase62(value);

        if (decoded.length > ID_LENGTH) {
            throw new IllegalArgumentException("Invalid Id length");
        }

        // pad về đúng 20 bytes
        byte[] result = new byte[ID_LENGTH];
        System.arraycopy(decoded, 0, result, ID_LENGTH - decoded.length, decoded.length);
    ...
    private static String encodeBase62(byte[] input) {
        ...
        char[] buffer = new char[32]; // đủ cho 160-bit
        ...
        return pos == buffer.length ? "0" : new String(buffer, pos, buffer.length - pos);
    }
    ...
    private static String leftPad(String input, int length, char padChar) {
